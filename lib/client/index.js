module.exports = function multiplex(primus) {
  ...
  Multiplex.prototype.transform = function () {
    ...
    multiplex.primus.transform('incoming', function (packet) {
      var data = packet.data, id, type, topic, channel;
      if (isArray(data)) {
        type = data.shift();
        id = data.shift();
        topic = data.shift();
        console.log('INCOMING', type, id, topic, data, !multiplex.channels[topic]);
        if (!(multiplex.channels[id])) return true;
        switch (type) {
          case types.UNSUBSCRIBE:
            multiplex.onunsubscribe(id);
            break;
          case types.MESSAGE:
            multiplex.onmessage(id, data);
            break;
        }
        return false;
      }
