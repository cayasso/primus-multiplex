...
Channel.prototype.subscribe = function (conn, id) {
  ...
  //this._conns[conn.id] = this._conns[conn.id] || [];
  //this._conns[conn.id].push(spark.id);
  ...
  console.log('SUBSCRIBING', conn.id, conn.channels);
  return this;
};

/**
 * Remove stail main connections by unsubscribing all child 
 * connections. This is important when for example when a 
 * client closes his browser and was subscribed to multiple channels.
 *
 * @param {String|Number} id The main connection id.
 * @return {Channel} self.
 * @api private
 */

Channel.prototype.removeStailConnections = function (id) {
  var ids = this._conns[id];
  for (var i = 0; i < ids.length; i++) {
    this.unsubscribe(ids[i]);
  }
