...
Multiplex.prototype.ondisconnection = function (conn) {
  var channel, id, spark;
  for (var name in conn.channels) {
    if (!(channel = this.channels[name])) continue;
    for (var i = 0; i < conn.channels[name].length; i++) {
      id = conn.channels[name][i];
      spark = channel.connections[id];
      if (spark) spark.end();
    }
  }
  return this;
